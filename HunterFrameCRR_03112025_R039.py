...
##### R039
data_R039 = open_fits(r"C:\Users\Grant Mondeel\Box\CfA\Xray Crystal\Data\R039_5000eV_35mA_ArInjec_900s_0p634sinxt.fits")
R039 = HunterCrr(data_R039, do_Grant_CRR=True)
L=len(R039)
collapsedAvgR039 = list(np.zeros(int((windowWidth-1)/2))) #make two empty entries to 

for window in [np.linspace(a,a+windowWidth-1,num=windowWidth) for a in np.linspace(0,L-windowWidth,num=L-windowWidth+1)]:
    collapsedAvgR039.append(np.sum([R039[windowInd] for windowInd in window.astype(int)])/windowWidth)
collapsedAvgR039 = np.array(collapsedAvgR039)
smoothR039 = gaussian_filter(R039, sigma=Sigma)


##### CRR analysis using all available data
# listOfData = [data_R039]
...
collapsedData = {"R039":{"data":R039,"avg":collapsedAvgR039,"smooth":smoothR039},
                 "All":{"data":R039,"avg":collapsedAvgR039,"smooth":smoothR039}}
...
if False:
    ...
    with open("03112025_R039.txt", "w", newline='') as f:
        writer = csv.writer(f)
