...
##### R036
data_R036 = open_fits(r"C:\Users\Grant Mondeel\Box\CfA\Xray Crystal\Data\R036_5000eV_35mA_ArInject_00.fits")
R036 = HunterCrr(data_R036, do_Grant_CRR=True)
L=len(R036)
collapsedAvgR036 = list(np.zeros(int((windowWidth-1)/2))) #make two empty entries to 

for window in [np.linspace(a,a+windowWidth-1,num=windowWidth) for a in np.linspace(0,L-windowWidth,num=L-windowWidth+1)]:
    collapsedAvgR036.append(np.sum([R036[windowInd] for windowInd in window.astype(int)])/windowWidth)
collapsedAvgR036 = np.array(collapsedAvgR036)
smoothR036 = gaussian_filter(R036, sigma=Sigma)


##### CRR analysis using all available data
# listOfData = [data_R036]
...
collapsedData = {"R036":{"data":R036,"avg":collapsedAvgR036,"smooth":smoothR036},
                 "All":{"data":R036,"avg":collapsedAvgR036,"smooth":smoothR036}}
...
if False:
    ...
    with open("03102025_R036.txt", "w", newline='') as f:
        writer = csv.writer(f)
