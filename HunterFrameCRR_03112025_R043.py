...
##### R043
data_R043 = open_fits(r"C:\Users\Grant Mondeel\Box\CfA\Xray Crystal\Data\R043_5000eV_30mA_ArInjec_1200s_0p637sinx.fits")
R043 = HunterCrr(data_R043, do_Grant_CRR=True)
L=len(R043)
collapsedAvgR043 = list(np.zeros(int((windowWidth-1)/2))) #make two empty entries to 

for window in [np.linspace(a,a+windowWidth-1,num=windowWidth) for a in np.linspace(0,L-windowWidth,num=L-windowWidth+1)]:
    collapsedAvgR043.append(np.sum([R043[windowInd] for windowInd in window.astype(int)])/windowWidth)
collapsedAvgR043 = np.array(collapsedAvgR043)
smoothR043 = gaussian_filter(R043, sigma=Sigma)


##### CRR analysis using all available data
# listOfData = [data_R043]
...
collapsedData = {"R043":{"data":R043,"avg":collapsedAvgR043,"smooth":smoothR043},
                 "All":{"data":R043,"avg":collapsedAvgR043,"smooth":smoothR043}}
...
if False:
    ...
    with open("03112025_R043.txt", "w", newline='') as f:
        writer = csv.writer(f)
