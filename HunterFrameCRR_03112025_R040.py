...
##### R040
data_R040 = open_fits(r"C:\Users\Grant Mondeel\Box\CfA\Xray Crystal\Data\R040_5000eV_30mA_ArInjec_900s_0p635sinxt.fits")
R040 = HunterCrr(data_R040, do_Grant_CRR=True)
L=len(R040)
collapsedAvgR040 = list(np.zeros(int((windowWidth-1)/2))) #make two empty entries to 

for window in [np.linspace(a,a+windowWidth-1,num=windowWidth) for a in np.linspace(0,L-windowWidth,num=L-windowWidth+1)]:
    collapsedAvgR040.append(np.sum([R040[windowInd] for windowInd in window.astype(int)])/windowWidth)
collapsedAvgR040 = np.array(collapsedAvgR040)
smoothR040 = gaussian_filter(R040, sigma=Sigma)


##### CRR analysis using all available data
# listOfData = [data_R040]
...
collapsedData = {"R040":{"data":R040,"avg":collapsedAvgR040,"smooth":smoothR040},
                 "All":{"data":R040,"avg":collapsedAvgR040,"smooth":smoothR040}}
...
if False:
    ...
    with open("03112025_R040.txt", "w", newline='') as f:
        writer = csv.writer(f)
