...
##### R038
data_R038 = open_fits(r"C:\Users\Grant Mondeel\Box\CfA\Xray Crystal\Data\R038_5000eV_35mA_ArInject.fits")
R038 = HunterCrr(data_R038, do_Grant_CRR=True)
L=len(R038)
collapsedAvgR038 = list(np.zeros(int((windowWidth-1)/2))) #make two empty entries to 

for window in [np.linspace(a,a+windowWidth-1,num=windowWidth) for a in np.linspace(0,L-windowWidth,num=L-windowWidth+1)]:
    collapsedAvgR038.append(np.sum([R038[windowInd] for windowInd in window.astype(int)])/windowWidth)
collapsedAvgR038 = np.array(collapsedAvgR038)
smoothR038 = gaussian_filter(R038, sigma=Sigma)


##### CRR analysis using all available data
# listOfData = [data_R038]
...
collapsedData = {"R038":{"data":R038,"avg":collapsedAvgR038,"smooth":smoothR038},
                 "All":{"data":R038,"avg":collapsedAvgR038,"smooth":smoothR038}}
...
if False:
    ...
    with open("03112025_R038.txt", "w", newline='') as f:
        writer = csv.writer(f)
